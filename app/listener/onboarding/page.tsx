...
export default function ListenerOnboardingPage() {
  ...
  const handleContinue = async () => {
    if (step === 1) {
      ...
      // Verification successful, redirect to preferences
      router.push("/listener/preferences")
    }
  }
    ...
    <main className="flex min-h-[100svh] flex-col bg-background p-4">
      ...
      <Card className="mx-auto w-full max-w-md border-none bg-transparent shadow-none">
        <CardHeader className={step === 1 ? "" : "text-center"}>
          <CardTitle>
            {step === 1 ? "Join as a Listener" : step === 2 ? "Email Verification" : "Email Verified Successfully"}
          </CardTitle>
          <CardDescription>
            {step === 1
              ? "Listen & Earn: Get paid to provide feedback on music"
              : step === 2
